...
# Luetaan avaimet turvallisesti GitHubin asetuksista (ympäristömuuttujista)
    ...
    try:
        ...
        for item in response.get("items", []):
            if "id" in item and "videoId" in item["id"]:
                liiga_videot.append({
                    "otsikko": item["snippet"].get("title", "Liiga-video"),
                    "url": f"https://youtube.com{item['id']['videoId']}",
...
def generoi_juonto_ilmaiseksi(video_url, videon_kuvaus):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="Qwen/Qwen2.5-72B-Instruct", messages=messages, max_tokens=500, temperature=0.7
        )
...
def aja_automaatio():
    ...
    for video in videot:
        ...
        valmiit_leikkeet.append({"otsikko": video["otsikko"], "url": video["url"], "juonto": juonto})
    try:
